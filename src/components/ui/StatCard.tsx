    ...
    <section className="card animate-rise group relative overflow-hidden p-5 sm:p-6">
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-xs font-black uppercase tracking-[.16em] text-slate-600 dark:text-slate-300">{title}</p>
          <h3 className="mt-2 truncate text-2xl font-black tracking-[-.04em] text-slate-950 dark:text-white sm:text-3xl">{value}</h3>
          {hint && <p className="mt-2 line-clamp-2 text-xs font-semibold text-slate-600 dark:text-slate-300">{hint}</p>}
        </div>
